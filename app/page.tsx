...
const DEFAULT_PRODUCTS: Product[] = [
  {
    ...
    description:
      "Persönlicher Sicherheitsalarm mit lauter Sirene zur Abschreckung und Aufmerksamkeitserzeugung in Gefahrensituationen.",
    price: 24.9,
    image: "/alarm.jpg",
    badge: "Bestseller",
    category: "Alarm",
    features: ["Laut", "Kompakt", "Alltagstauglich"],
    active: true,
    stock: 20,
    sku: "HS-001",
  },
  {
    ...
    description:
      "Kompaktes Sicherheitslicht zur besseren Sichtbarkeit bei Nacht und unterwegs.",
    price: 29.9,
    image: "/light.jpg",
    badge: "Top Wahl",
  ...
  },
  {
    id: 3,
    title: "Safety Clip",
    subtitle: "Klein, diskret und praktisch",
    description:
      "Kompaktes Sicherheits-Accessoire für Alltag und Heimweg. Leicht mitzunehmen und einfach zu befestigen.",
    price: 19.9,
    image: "/spray.jpg",
    badge: "Neu",
    category: "Accessoire",
    features: ["Diskret", "Leicht", "Praktisch"],
    active: true,
    stock: 30,
    sku: "HS-003",
  },
...
export default function HelvSafeLandingPage() {
  ...
  const saveProduct = async () => {
    ...
    const payload = editingProduct ? { ...basePayload, id: productForm.id } : basePayload;
  ...
  const placeOrder = async () => {
    ...
    if (paymentMethod === "invoice") {
      ...
      try {
        ...
        if (error) {
          console.error(error);
          alert("Rechnungsbestellung konnte nicht gespeichert werden.");
          return;
        }
        ...
        alert("Bestellung auf Rechnung wurde aufgenommen. Du erhältst die Rechnung separat.");
        return;
      } catch (error) {
        console.error(error);
        alert("Fehler bei Rechnungsbestellung");
        return;
      } finally {
    ...
    try {
      const res = await fetch(BACKEND_CONFIG.checkoutUrl, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ provider: paymentMethod, order: newOrder }),
      });
      ...
      <div className="min-h-screen bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_45%,#eef2f7_100%)] text-slate-900">
        ...
        <main>
          <section className="mx-auto grid max-w-7xl items-center gap-12 px-6 py-16 lg:grid-cols-[1.02fr_0.98fr] lg:px-8 lg:py-24">
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-xl">
              ...
              <h1 className="text-5xl font-semibold leading-[0.98] tracking-tight text-slate-950 sm:text-6xl lg:text-7xl">
                Fühl dich sicher,
                <br />
                <span className="bg-gradient-to-r from-slate-900 via-slate-700 to-red-500 bg-clip-text text-transparent">heute noch.</span>
              </h1>
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                <a href="#produkte" className="rounded-full bg-gradient-to-r from-red-500 to-red-600 px-8 py-3.5 text-sm font-medium text-white shadow-xl shadow-red-200 transition hover:scale-[1.02]">Produkte ansehen</a>
                <button onClick={() => setCheckoutOpen(true)} className="rounded-full border border-slate-200 bg-white px-8 py-3.5 text-sm font-medium text-slate-900 shadow-sm transition hover:bg-slate-50">Diskret bestellen</button>
              </div>

              <div className="mt-8 flex flex-wrap gap-3 text-sm text-slate-600">
                <span className="rounded-full bg-white px-3 py-1.5 shadow-sm ring-1 ring-slate-200">{SHOP_SETTINGS.shippingNotice}</span>
                <span className="rounded-full bg-white px-3 py-1.5 shadow-sm ring-1 ring-slate-200">Sichere Bestellung • Payrexx • TWINT • Kreditkarte</span>
              </div>
            </motion.div>

            <motion.div initial={{ opacity: 0, scale: 0.98, x: 14 }} animate={{ opacity: 1, scale: 1, x: 0 }} transition={{ duration: 0.6 }} className="relative">
              ...
              <div className="relative overflow-hidden rounded-[2.25rem] border border-white/80 bg-white/90 p-3 shadow-[0_35px_90px_rgba(15,23,42,0.16)] backdrop-blur-xl">
                ...
                <img src="/hero.jpg" alt="Selbstbewusste Frau" className="block h-[560px] w-full rounded-[1.75rem] object-cover object-[center_top] shadow-[0_40px_100px_rgba(0,0,0,0.25)] transition duration-700 hover:scale-[1.02]" />
              </div>
          ...
          <section id="produkte" className="mx-auto max-w-7xl px-6 py-12 lg:px-8">
            <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              ...
              <div className="relative w-full max-w-md md:w-auto">
                ...
                <input value={productSearch} onChange={(e) => setProductSearch(e.target.value)} placeholder="Produkte suchen" className="w-full rounded-full border border-slate-200 bg-white px-11 py-3 text-sm outline-none transition focus:border-red-300" />
              </div>
            </div>

            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {activeProducts.map((product, index) => (
                <motion.div key={product.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }} className="group overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_50px_rgba(15,23,42,0.12)]">
                  <div className="relative overflow-hidden rounded-[1.25rem] bg-slate-50">
                    ...
                    <div className="absolute left-4 top-4 rounded-full border border-white/80 bg-white/95 px-3 py-1 text-xs font-medium text-red-600 shadow-sm">{product.badge}</div>
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {product.features.map((feature) => (
                      <span key={feature} className="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">{feature}</span>
                    ))}
                  </div>
                  ...
                  <div className="mt-5 flex items-center justify-between gap-3">
                    ...
                    <button onClick={() => addToCart(product)} disabled={product.stock <= 0} className="rounded-full bg-gradient-to-r from-red-500 to-red-600 px-4 py-2 text-sm font-medium text-white shadow-lg shadow-red-200 transition hover:scale-[1.02] disabled:opacity-50 disabled:hover:scale-100">
          ...
          <section id="warenkorb" className="mx-auto max-w-7xl px-6 pb-20 lg:px-8">
            <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
              ...
              <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div className="mt-6 space-y-3 text-sm">
                  <div className="rounded-[1rem] bg-slate-50 p-4">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => setShippingMethod("standard")} className={`rounded-full px-3 py-2 text-xs ${shippingMethod === "standard" ? "bg-gradient-to-r from-red-500 to-red-600 text-white" : "bg-slate-200 text-slate-700"}`}>Standard</button>
                      <button onClick={() => setShippingMethod("express")} className={`rounded-full px-3 py-2 text-xs ${shippingMethod === "express" ? "bg-gradient-to-r from-red-500 to-red-600 text-white" : "bg-slate-200 text-slate-700"}`}>Express</button>
                    </div>
                  </div>
                ...
                <button onClick={() => setCheckoutOpen(true)} className="mt-6 w-full rounded-full bg-gradient-to-r from-red-500 to-red-600 px-5 py-3 text-sm font-medium text-white shadow-lg shadow-red-200 transition hover:scale-[1.01]">Zur Bestellung</button>

                <div className="mt-5 rounded-[1rem] bg-slate-50 p-4">
                  <div className="mb-3 text-sm font-medium">Zahlungsart</div>
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => setPaymentMethod("twint")} className={`rounded-full px-3 py-2 text-xs ${paymentMethod === "twint" ? "bg-gradient-to-r from-red-500 to-red-600 text-white" : "bg-slate-200 text-slate-700"}`}>TWINT</button>
                    <button onClick={() => setPaymentMethod("card")} className={`rounded-full px-3 py-2 text-xs ${paymentMethod === "card" ? "bg-gradient-to-r from-red-500 to-red-600 text-white" : "bg-slate-200 text-slate-700"}`}>Kreditkarte</button>
                    <button onClick={() => setPaymentMethod("invoice")} className={`rounded-full px-3 py-2 text-xs ${paymentMethod === "invoice" ? "bg-gradient-to-r from-red-500 to-red-600 text-white" : "bg-slate-200 text-slate-700"}`}>Rechnung</button>
                  </div>
                  <div className="mt-3 text-xs text-slate-500">TWINT und Kreditkarte werden über Payrexx abgewickelt. Bei Auswahl von Rechnung wird die Bestellung direkt gespeichert und anschliessend manuell bearbeitet.</div>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="mx-auto max-w-7xl px-6 py-20 lg:px-8">
            <div id="kontakt" className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm">
              <div className="max-w-2xl">
                ...
                <p className="mt-3 text-sm leading-7 text-slate-600">Für Fragen zu Versand, Produkten oder diskreter Bestellung. Bei akuter Gefahr nutze bitte zuerst den Notruf 117 oder die offizielle Opferhilfe.</p>
              </div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <button onClick={() => setAdminOpen(true)} className="rounded-full border border-slate-200 bg-white px-5 py-2.5 text-sm font-medium text-slate-700 transition hover:border-red-300 hover:text-red-500">Interner Adminbereich</button>
              </div>
              ...
              <div className="mt-6 space-y-3">
                ...
                <button onClick={sendContactForm} className="rounded-full bg-gradient-to-r from-red-500 to-red-600 px-6 py-3 text-sm font-medium text-white shadow-lg shadow-red-200">{contactLoading ? "Wird gesendet..." : "Nachricht senden"}</button>
          ...
          <div className="mx-auto grid max-w-7xl gap-8 px-6 py-10 text-sm text-slate-500 md:grid-cols-4 lg:px-8">
            ...
            <div>
              ...
              <div className="mt-3 space-y-2">
                ...
                <button onClick={() => setAdminOpen(true)} className="text-sm text-slate-500 transition hover:text-red-500">Interner Adminbereich</button>
              </div>
            </div>
            <div>
            ...
            <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] bg-white p-6 shadow-2xl sm:p-8">
              <div className="mb-6 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">Bei TWINT und Kreditkarte wirst du sicher zu Payrexx weitergeleitet. Bei Auswahl von Rechnung wird die Bestellung direkt gespeichert und anschliessend manuell bearbeitet.</p>
                </div>
              ...
              <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
                ...
                <div className="rounded-[1.75rem] bg-slate-50 p-5">
                    ...
                    {cart.length === 0 ? (
                      ...
                      cart.map((item) => (
                        <div key={item.id} className="flex items-center justify-between gap-3">
                          <div>
                            <div className="font-medium">{item.title}</div>
                            <div className="text-slate-500">Menge: {item.qty}</div>
                          </div>
                  ...
                  <button onClick={placeOrder} disabled={checkoutLoading} className="mt-6 flex w-full items-center justify-center gap-2 rounded-full bg-gradient-to-r from-red-500 to-red-600 px-5 py-3 text-sm font-medium text-white shadow-lg shadow-red-200 disabled:opacity-60">
                    ...
                    {checkoutLoading ? "Wird verarbeitet..." : paymentMethod === "twint" ? "Mit TWINT weiter" : paymentMethod === "card" ? "Mit Karte weiter" : "Bestellung abschicken"}
                  </button>
                  <p className="mt-3 text-xs leading-6 text-slate-500">Bei TWINT und Kreditkarte wirst du sicher zu Payrexx weitergeleitet. Bei Auswahl von Rechnung wird die Bestellung direkt gespeichert und anschliessend manuell bearbeitet.</p>
                </div>
              </div>
            </div>
          </div>
                ...
                <div className="grid gap-8 lg:grid-cols-[1fr_1fr]">
                  <div>
                    ...
                    <div className="space-y-3 rounded-[1.5rem] border border-slate-200 p-5">
                      ...
                      <input value={productForm.image} onChange={(e) => setProductForm((d) => ({ ...d, image: e.target.value }))} placeholder="Bild-URL" className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-red-300" />
            ...
            <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] bg-white p-6 shadow-2xl sm:p-8">
              ...
              {legalPage === "datenschutz" && (
                <div className="space-y-3 text-sm leading-7 text-slate-600">
                  ...
                  <p>Die Zahlungsabwicklung erfolgt über den Zahlungsdienstleister Payrexx. Dabei werden die für die Zahlung erforderlichen Daten an Payrexx weitergegeben.</p>
                </div>
              )}
              {legalPage === "agb" && (
                <div className="space-y-3 text-sm leading-7 text-slate-600">
                  ...
                  <p><strong>4. Zahlung</strong><br />Die Zahlung erfolgt über die im Checkout angebotenen Zahlungsmethoden.</p>
                </div>
              )}
            </div>
          </div>
